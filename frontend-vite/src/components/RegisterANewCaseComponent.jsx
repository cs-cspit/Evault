...
const RegisterNewCaseComponent = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Get the current lawyer's info from storage
      const userType = secureStorage.getItem("userType");
      const userUID = secureStorage.getItem("userUID");

      console.log("Current user info:", { userType, userUID });

      if (!userType || !userUID) {
        throw new Error("User not found in storage");
      }

      if (userType.toLowerCase() !== "lawyer") {
        throw new Error(`Only lawyers can register cases. Current user type: ${userType}`);
      }

      console.log("Registering case with lawyer UID:", userUID);

      // Register the case with the current lawyer's UID
      await registerNewCase({
        UIDOfParty1: formData.party1UID,
        UIDOfParty2: formData.party2UID,
        caseSubject: formData.caseTitle,
        associatedLawyers: [userUID] // Include the current lawyer's UID
      });
        ...
        <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl p-6 md:p-8">
          ...
          {clients.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-600">No registered clients found. Please register clients first.</p>
            </div>
          ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    >
                      ...
                      {clients.map((client) => (
                        <option key={`p1-${client.uid}`} value={client.uid}>
                    ...
                    >
                      ...
                      {clients.map((client) => (
                        <option 
                          key={`p2-${client.uid}`} 
                          value={client.uid}
                          disabled={client.uid === formData.party1UID}
                        >
                          {client.name} (UID: {client.uid})
                        </option>
                      ))}
